...
import { CheckIcon, TruckIcon, MapPinIcon, Loader2 } from "lucide-react";
...
function Confirmation() {
  ...
  // so this view auto-swaps to the success state the moment status flips.
      ...
      <div className="w-full flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="w-full lg:w-[65%] border border-[#e5e7eb] rounded-[12px] p-5 md:p-[32px]">
            ...
            {order.items.map((item, index) => (
              <div key={index}>
                <div className="flex items-start gap-[16px]">
                  <div className="w-[90px] h-[90px] rounded-[8px] overflow-hidden flex-shrink-0 bg-gray-100">
          ...
          <div className="flex flex-col sm:flex-row gap-[14px] mt-[28px]">
            <button className="flex-1 flex items-center justify-center gap-[8px] bg-[#00677F] hover:bg-[#005569] text-white font-semibold py-[14px] px-[20px] rounded-[8px] transition-colors cursor-pointer">
        ...
        <section className="w-full py-[80px] px-5 lg:px-0">
          ...
          <div className="flex lg:grid lg:grid-cols-4 overflow-x-auto gap-4 lg:gap-6 scrollbar-hide snap-x -mx-5 px-5 lg:mx-0 lg:px-0 pb-6 lg:pb-0">
            {recommendations.map((item) => (
              ...
              >
                <div className="w-full aspect-square rounded-[8px] overflow-hidden mb-[14px] bg-gray-100">
                  <img src={item.images[0]} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-out" />
                </div>
